...
//number will place cirles at a faster rate, but spread out. 
...
void setup() {
  size(640, 480);
  circleX = width/2;//starting location of the X axis of the circle. The starting location of the X axis is 320px
  ...
  background(BACKGROUND_COLOR);//defines the color of the background, pink.
}

void draw() {
    if (dist(mouseX, mouseY, circleX, circleY) < CIRCLE_SIZE/2) { //if the distance of the mouse on the X and Y axis 
